...
class FCOSDanceHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.loss_ext = build_loss(loss_ext)

    def _init_layers(self):
        """Initialize layers of the head."""
        super()._init_layers()
        self.conv_centerness = nn.Conv2d(self.feat_channels, 1, 3, padding=1)
        self.conv_ext = nn.Conv2d(self.feat_channels, 4, 3, padding=1)
    ...
    def forward_single(self, x, scale, stride):
        ...
        return cls_score, bbox_pred, centerness, extremeness

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'extremenesses'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        labels, bbox_targets, ext_targets = self.get_targets(all_level_points, gt_bboxes,
                                                gt_labels, gt_masks)
        ...
        #ext
        flatten_ext_preds = [
            extremeness.permute(0, 2, 3, 1).reshape(-1, 4)
            for extremeness in extremenesses
        ]
        flatten_cls_scores = torch.cat(flatten_cls_scores)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds)
        flatten_centerness = torch.cat(flatten_centerness)
        flatten_ext_preds = torch.cat(flatten_ext_preds)
        flatten_labels = torch.cat(labels)
        flatten_bbox_targets = torch.cat(bbox_targets)
        #ext_targets
        flatten_ext_targets = torch.cat(ext_targets)
        ...
        pos_ext_preds = flatten_ext_preds[pos_inds]
        pos_centerness = flatten_centerness[pos_inds]
        pos_bbox_targets = flatten_bbox_targets[pos_inds]
        pos_ext_targets = flatten_ext_targets[pos_inds]
        ...
        if len(pos_inds) > 0:
            ...
            pos_decoded_ext_preds = self.bbox_coder.decode(
                pos_points, pos_ext_preds)
            pos_decoded_target_ext_preds = self.bbox_coder.decode(
                pos_points, pos_ext_targets)
            loss_bbox = self.loss_bbox(
                pos_decoded_bbox_preds,
                pos_decoded_target_preds,
                weight=pos_centerness_targets,
                avg_factor=centerness_denorm)
            loss_ext = self.loss_ext(
                pos_decoded_ext_preds,
                pos_decoded_target_ext_preds,
                weight=pos_centerness_targets,
                avg_factor=centerness_denorm)
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list, gt_masks_list):
        ...
        labels_list, bbox_targets_list, ext_targets_list = multi_apply(
        ...
        ext_targets_list = [
            ext_targets.split(num_points, 0)
            for ext_targets in ext_targets_list
        ]

        # concat per level image
        concat_lvl_labels = []
        concat_lvl_bbox_targets = []
        concat_lvl_ext_targets = []
        for i in range(num_levels):
            ...
            concat_lvl_ext_targets.append(ext_targets)
        return concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_ext_targets
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, gt_masks, points, regress_ranges,
        ...
        if num_gts == 0:
            return gt_labels.new_full((num_points,), self.num_classes), \
                   ...
                   gt_masks.new_zeros((num_points, 4))
